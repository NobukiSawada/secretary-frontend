                ...
                <div className="plan-events">
                  {plan.events.map((event, eventIndex) => (
                    <div key={eventIndex} className="plan-event-item">
                      ...
                      <p>{new Date(event.start_time).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })} - {new Date(event.end_time).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}</p>
                      {event.location && <p>場所: {event.location}</p>}
                      {event.description && <p>{event.description}</p>}
                    </div>
                  ))}
                </div>
                <button className="select-plan-button" onClick={() => onSelectPlan(plan.events)}>このプランを追加</button>
              </div>
            ))}
          </div>
        )}
